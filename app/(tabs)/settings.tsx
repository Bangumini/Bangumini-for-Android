...
export default function SettingsPage() {
	...
	const { checking, loggedIn, username, loginWithToken, logout, refresh } =
		useAuth();
	...
	async function clearExpiredCache() {
		try {
			...
			alert(
				"清理完成",
				`已清理过期缓存${expiredImages.length ? `，图片记录 ${expiredImages.length} 条` : ""}`,
			);
		} catch (error) {
	...
	async function handleDownload() {
		...
		try {
			...
			// 已授权 -> 仅保留是否安装的选项；未授权 -> 提示去授权
			if (await canRequestPackageInstalls()) {
				alert("下载完成", "是否现在安装新版本？", [
					{ text: "开始安装", onPress: () => void doInstall(fileUri) },
				]);
			} else {
				alert("下载完成", "需要开启「安装未知应用」权限才能安装更新", [
					{
						text: "去授权",
	...
	return (
		<ScrollView style={styles.screen} contentContainerStyle={styles.content}>
			<View style={styles.section}>
				...
				<Pressable
					...
					style={[
						...
						(!token.trim() || savingToken) && styles.disabled,
					]}
			...
			<View style={styles.section}>
				...
				<SettingsRow
					title="复制条目名保留季名"
					detail="关闭后会尝试去掉第 2 季、Season 2 等后缀"
				>
					<Switch
			...
			<View style={styles.section}>
				...
				<Pressable
					...
					onPress={() => void clearExpiredCache()}
				>
				...
				<Pressable
					...
					onPress={() =>
						void Linking.openURL(
							"https://github.com/Bangumini/Bangumini-for-Android",
						)
					}
				>
